...
function fixlinks!(ctx, navnode, link::Markdown.Link)
    ...
    if Compat.Sys.iswindows() && ':' in first(s)
        @error "Invalid local link '$(link.url)': (colons not allowed in paths on Windows)" # _file = navnode.page
        return
    end
    ...
    elseif isfile(joinpath(ctx.doc.user.build, path))
        ...
        @error "Invalid local link '$(link.url)' (unresolved path)" # _file = navnode.page
    end
...
function fixlinks!(ctx, navnode, img::Markdown.Image)
    ...
    if Compat.Sys.iswindows() && ':' in img.url
        @error "Invalid local image '$(img.url)' (colons not allowed in paths on Windows)" # _file = navnode.page
        return
    end
    ...
    if isfile(joinpath(ctx.doc.user.build, path))
        ...
        @error "Invalid local image '$(img.url)' (unresolved path" # _file = navnode.page
    end
end
